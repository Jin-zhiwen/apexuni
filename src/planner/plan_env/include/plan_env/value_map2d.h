...
public:
  ...
  double getBaseValue(const Vector2i& idx) const;
  double getObjectValue(const Vector2i& idx) const;
...
private:
  ...
  vector<double> object_value_buffer_;  // Decaying local object evidence
...
inline double ValueMap::getBaseValue(const Vector2i& idx) const
{
  int adr = sdf_map_->toAddress(idx);
  return value_buffer_[adr];
}

inline double ValueMap::getObjectValue(const Vector2i& idx) const
{
  int adr = sdf_map_->toAddress(idx);
  return object_value_buffer_[adr];
}
