...
private:
  ...
  double object_fov_angle_sigma_;
  double object_fov_range_sigma_;
  double object_fov_max_angle_;
  double object_fov_max_range_error_;
  double object_fov_weight_;
  double object_fov_decay_tau_;
...
inline double MapROS::smoothValueMapIntensity(const Eigen::Vector2i& idx) const
{
  ...
  constexpr double kVisualMinIntensity = 0.0;
  constexpr double kVisualMaxIntensity = 1.0;
  constexpr double kVisualRawMin = 0.08;
  constexpr double kVisualRawMax = 0.80;
  ...
  const double clamped = std::max(0.0, std::min(1.0, std::max(smoothed, decayed_peak)));
  const double visual_ratio =
      std::max(0.0, std::min(1.0, (clamped - kVisualRawMin) / (kVisualRawMax - kVisualRawMin)));
  const double visual_intensity =
      kVisualMinIntensity + (kVisualMaxIntensity - kVisualMinIntensity) * visual_ratio;

  return std::max(visual_floor, visual_intensity);
}
