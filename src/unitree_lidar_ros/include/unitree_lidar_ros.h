    ...
    bool run()
    {
        ...
        if (result == LIDAR_IMU_DATA_PACKET_TYPE)
        {
            ...
            if (lsdk_->getImuData(imu))
            {
                ...
                {
                    imuMsg.header.stamp = ros::Time(imu.info.stamp.sec, imu.info.stamp.nsec);
                }
                ...
                transform.setRotation(tf::Quaternion(imu.quaternion[0], imu.quaternion[1],
                                                     imu.quaternion[2], imu.quaternion[3]));
                tfbc1_.sendTransform(tf::StampedTransform(transform, imuMsg.header.stamp, imu_frame_ + "_initial", imu_frame_));

                // publish tf from imu to lidar
                transform.setOrigin(tf::Vector3(0.007698, 0.014655, -0.00667));
                transform.setRotation(tf::Quaternion(0, 0, 0, 1));
                tfbc1_.sendTransform(tf::StampedTransform(transform, imuMsg.header.stamp, imu_frame_, cloud_frame_));
